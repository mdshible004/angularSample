(function () {
    ...
    journalController.$inject = ['journalService', 'filterurl', '$q', 'authservice', 'commonService', 'conversion', 'chartOfAccountsService', 'logger', '$scope', '$localStorage', '$rootScope', '$state', 'Upload', '$timeout', '$http', '$interval', 'uiGridConstants', 'userAuthenticationService','apiConfig'];
    function journalController(journalService, filterurl, $q, authservice, commonService, conversion, chartOfAccountsService, logger, $scope, $localStorage, $rootScope, $state, Upload, $timeout, $http, $interval, uiGridConstants, userAuthenticationService, apiConfig) {
       ...
       var params = {};
            ...
            if (vm.JournalID === 0 && vm.ChequeSetup !== undefined) {
                ...
                if (vm.JournalTypeID === 3) {
                    ...
                    vm.JournalDetailList.push({
                        ...
                        COAID: null,
                        CL: { selected: null },
                        UserID: vm.ChequeSetup.UserID,
                        //USR: { selected: vm.UserList.filter(function (ob, i) { return (ob.UserID === vm.ChequeSetup.PartyID.toString()); })[0] },
                        //UserID: vm.ChequeSetup.PartyID,
                        UserName: vm.ChequeSetup.UserFullName,  //{ selected: vm.UserList.filter(function (ob, i) { return (ob.UserID === vm.ChequeSetup.PartyID.toString()); })[0] },  //detail.UserName,
                       
                        CostCenterID: vm.CostCenterID === undefined ? null : vm.CostCenterID,
                        BR: { selected: vm.BranchList[0] }, //{ selected: null },
                    ...
                    vm.JournalDetailList.push({
                        JournalID: vm.JournalID,
                        JournalDetailID: 0,
                        COAID: vm.BankAccountList.filter(function (ob, i) { return (ob.AccountID === vm.AccountID); })[0].COAID,
                        CL: { selected: vm.COAList.filter(function (ob, i) { return (ob.COAID === vm.BankAccountList.filter(function (ob, i) { return (ob.AccountID === vm.AccountID); })[0].COAID); })[0] },
                        UserID: null,
                        USR: { selected: null },
                        CostCenterID: null,
                        BR: { selected: null },// { selected: null },
                        DrAmount: 0,
                    ...
                    vm.JournalDetailList.push({
                        JournalID: vm.JournalID,
                        JournalDetailID: 0,
                        COAID: vm.BankAccountList.filter(function (ob, i) { return (ob.AccountID === vm.AccountID); })[0].COAID,  //null,
                        CL: { selected: vm.COAList.filter(function (ob, i) { return (ob.COAID === vm.BankAccountList.filter(function (ob, i) { return (ob.AccountID === vm.AccountID); })[0].COAID); })[0] },
                        UserID: null,
                        //UserID: vm.ChequeSetup.UserID,
                        //UserName: vm.ChequeSetup.UserFullName, 
                        USR: { selected: null },
                        CostCenterID: vm.CostCenterID === undefined ? null : vm.CostCenterID, //null,
                        BR: { selected: vm.BranchList[0] },//{ selected: null },
                        DrAmount: vm.ChequeSetup.ChequeAmount === 0 ? null : vm.ChequeSetup.ChequeAmount,
                        CrAmount: 0
                    });
        ...
        vm.loadBankAccountNo = function (modelState) {
            ...
            return journalService.getBankAccountList()
                .then(function (data) {
                    ...
                    if (modelState === 'Edit') {
                        vm.BA = { selected: vm.BankAccountList.filter(function (ob, i) { return (ob.AccountID === vm.AccountID); })[0] };
                    }
                });
        };
        loadCOAList();
        ...
        vm.addToList = function (model) {
            ...
            if (vm.JournalDetailList.length === 0) {
                ...
                //CL: { selected: $scope.IsDefaultCash.length === 0 ? null : $scope.isDefaultCashCheck },
                vm.JournalDetailList = [];
                ...
                //vm.JournalDetailList.push({ JournalID: 0, JournalDetailID: 0, COAID: vm.COAList.filter(function (ob, i) { return (ob.IsDefaultCash === true); })[0].COAID, CL: { selected: vm.COAList.filter(function (ob, i) { return (ob.COAID === vm.COAList.filter(function (ob, i) { return (ob.IsDefaultCash === true); })[0].COAID); })[0] }, 
                //    UserID: '', CostCenterID: vm.BranchList.length === 0 ? $scope.InsModel.BrunchID : vm.BranchList[0].BrunchID, BR: { selected: vm.BranchList.length === 0 ? $scope.InsModel : $scope.InsModelBranch }, DrAmount: null, CrAmount: null
                //});
                
               //vm.JournalDetailList.push({ JournalID: 0, JournalDetailID: 0, COAID: null, UserID: '', CostCenterID: vm.BranchList.length === 0 ? $scope.InsModel.BrunchID : vm.BranchList[0].BrunchID, BR: { selected: vm.BranchList.length === 0 ? $scope.InsModel : $scope.InsModelBranch }, DrAmount: null, CrAmount: null });
            }
        ...
        vm.Save = function () {
            ...
            //vm.JournalDetail.length;
            //console.log(vm.JournalDetail.length)
            //Generate Token API Pass Call
            authservice.getToken(generateSecurityToken(vm.InstituteID, vm.LoggedUserID, vm.menuId, vm.postMethod)); //jshint ignore : line
            //|| (JDetail.COAID !== undefined)
            //&& ($scope.detailsCount >= 2)
            if (($scope.ttlDrAmount === $scope.ttlCrAmount) && ($scope.ttlDrAmount !== undefined && $scope.ttlDrAmount !== 0) && ($scope.ttlCrAmount !== undefined && $scope.ttlCrAmount !== 0)) {

                $scope.manageMaster();
                $scope.manageChequeModal();
                $scope.manageDetailList();
               
                var FuncName = '/postAccJournalMasterDetail';
                return journalService.InsertJournalData(FuncName, vm)
                    .then(function (data) {
        ...
        $scope.ClearChequeModal = function () {
            ...
            vm.ChequeSetup.USR = { selected: null };
 ...
 // ___________________ Print End ___________________________
        ...
        activate();
        function activate() {
            var promises = [loadVoucherList(), loadCurrencyList(), loadCOAList(), loadBranchList(), loadPartyList()];
