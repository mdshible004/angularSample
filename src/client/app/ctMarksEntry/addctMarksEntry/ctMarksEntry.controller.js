...
(function () {
    ...
    function CtMarksEntryController(syllabusSevice, ClassTestService, branchSettingsSevice, insExameSetting, periodSetup, classSettingsService, subjectSettingsSevice, commonService, conversion, teacherAttendanceSevice, filterurl, $q, authservice, logger, $scope, $rootScope, $state, Upload, $timeout, $http, $interval, uiGridConstants, $localStorage) {
        ...
        //                && js.SyllabusID === row.SyllabusID
        //                && js.ClassDate === row.ClassDate
        //                && js.ClsDay === row.ClsDay && js.Topic === row.Topic
        //                && js.MainIndex === $scope.MainDetailIndex);
        //        })[0];
        ...
        //            && js.SyllabusID === model.SyllabusID
        //            && js.ClassDate === model.ClassDate
        //            && js.ClsDay === model.ClsDay && js.Topic === model.Topic
        ...
        vm.setInsExamCTMarks = function () {
            ...
            var CtArray = [].concat.apply([], vm.studentsArr);

            ClassTestService.setInsExamCTMarks({                
                CTMarkID: vm.CTMarkID,
                InsCTID: vm.InsCTID,
                StudentID: null,
                ObtainMarks: null,
        ...
        vm.getCTMarksByID = function (id) {

            var Params = {
                InsCTID: id

            };
            return ClassTestService.getCTMarksByID(Params)
                .then(function (data) {
                    vm.marks = data;
                    vm.Marks = data[0].CTMarks;
                    $scope.IsDisabled = false;
                });

        }; 

        vm.getCTListDDL = function () {
            //if (status === '') {
            //    $scope.ReloadMedium(1);
            //    $scope.ReloadDll(InstituteID);
            //}
            ...
            var Params = {
                ...
                SubjectID: vm.SubjectID === undefined ? null : vm.SubjectID,
                ExamID: vm.ExamID === undefined ? null : vm.ExamID,
                SessionID: vm.SessionID === undefined ? null : vm.SessionID,
            ...
            return ClassTestService.getCTListDDL(Params)
                .then(function (data) {
                    vm.Ctddl = data;
        ...
        $scope.diss = true;
        vm.getAllStudentsByParamsForCT = function () {
            //if (status === '') {
            //    $scope.ReloadMedium(1);
            //    $scope.ReloadDll(InstituteID);
            //}
            ...
            var Params = {
                ...
                DepartmentID: vm.DepartmentID === undefined ? null : vm.DepartmentID,
                SectionID: vm.SectionID === undefined ? null : vm.SectionID,
                SessionID: vm.SessionID === undefined ? 3 : vm.SessionID,
                BranchID: vm.BrunchID === undefined ? null : vm.BrunchID,
                BoardID: vm.BoardID === undefined ? null : vm.BoardID,
                ShiftID: vm.ShiftID === undefined ? null : vm.ShiftID,
                InsCTID: vm.InsCTID === undefined ? null : vm.InsCTID
            };
            return ClassTestService.getAllStudentsByParamsForCT(Params)
                .then(function (data) {
                    if (data.length > 0) {
                        $scope.diss = false;
                        vm.students = data;
                            ...
                            newrow =
                                {
                                    ...
                                    'ObtainMarks': value.ObtainMarks,
                                    'IsAbsent': value.IsAbsent,
                                    'InsCTID': value.InsCTID,
                                    'CTMarkID': value.CTMarkID
                                };

                            vm.studentsArr.push(newrow);
                        }
                    }
